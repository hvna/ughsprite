...
pygame.sprite.DirtySprite(*groups):
return DirtySprite
...
set
to
0, it is not dirty and therefore
not repainted
again.

blendmode = 0
It
's the special_flags argument of Surface.blit; see the blendmodes in
the
Surface.blit
documentation
...
class for containers of sprites
...
class from this or the other groups below if
...
Any
AbstractGroup - derived
sprite
groups
act
like
sequences and support
iteration, len, and so
on.
...
'in'
operator, e.g.
'sprite in group', 'subgroup in group'.
...
simple
container
for Sprite objects.This class can be subclassed to
create
containers
with more specific behaviors.The constructor takes any
number
of
Sprite
arguments
...
class that tracks dirty updates


pygame.sprite.RenderUpdates(*sprites):
return RenderUpdates

This


class is derived from pygame.sprite.Group().It has an enhanced draw
...
pygame.sprite.OrderedUpdates(*spites):
return OrderedUpdates

This


class derives from pygame.sprite.RenderUpdates().It maintains
...
and an
integer
for the layer.The default layer is 0.
...
LayeredUpdates.add(*sprites, **kwargs):
return None
...
return a
list
with all sprites at that position
...
LayeredUpdates.get_sprite(idx):
return sprite

Raises
IndexOutOfBounds if the
idx is not within
range.
...
all
sprites
from a layer and return them as a
list

LayeredUpdates.remove_sprites_of_layer(layer_nr):
return sprites
...
LayeredUpdates.change_layer(sprite, new_layer):
return None
...
LayeredUpdates.move_to_front(sprite):
return None
...
LayeredUpdates.move_to_back(sprite):
return None
...
LayeredUpdates.get_sprites_from_layer(layer):
return sprites
...
LayeredUpdates.switch_layer(layer1_nr, layer2_nr):
return None
...
LayeredDirty
Group is
for DirtySprites; subclasses LayeredUpdates
...
group
requires
pygame.sprite.DirtySprite or any
sprite
that
has
the
following
attributes:
image, rect, dirty, visible, blendmode(see
doc
of
DirtySprite).
...
dirty
flag
technique and is therefore
faster
than
pygame.sprite.RenderUpdates if you
have
many
static
sprites.It
...
As
with the pygame.sprite.Group, you can specify some additional attributes
through
kwargs:
...
are
added
_time_threshold: treshold
time
for switching between dirty rect mode
and fullscreen
mode;
defaults
to
...
_time_threshold: treshold
time
for switching between dirty rect
mode and fullscreen
mode;
defaults
to
...
LayeredDirty.draw(surface, bgd=None):
return Rect_list
...
LayeredDirty.repaint_rect(screen_rect):
return None

screen_rect is in screen
coordinates.
...
LayeredDirty.set_clip(screen_rect=None):
return None
...
class works just like a regular group, but it only keeps a single
...
Tests
for collision between two sprites.Uses the pygame.Rect colliderect
function
to
calculate
the
collision.It is intended
to
be
passed as a
collided
...
class that checks for collisions using scaled rects


The


class checks for collisions between two sprites using a scaled version


of
the
sprites
' rects. Is created with a ratio; the instance is then
intended
to
be
passed as a
...
pygame.sprite.collide_rect_ratio(ratio)(left, right):
return bool

Tests
for collision between two sprites.Uses the pygame.Rect
colliderect
function
to
calculate
...
Tests
for collision between two sprites by testing whether two circles
centered
on
the
sprites
...
class checks for collisions between two sprites using a
...
a
sprite
's radius. It is created with a ratio as the
argument
to
the
constructor.The
...
pygame.sprite.collide_circle_radio(ratio)(left, right):
return bool
...
Tests
for collision between two sprites by testing if their bitmasks
overlap.If
the
sprites
have
...
intersect
with another
    Sprite.Intersection is determined
    by
    comparing
    the
    Sprite.rect
...
take
two
sprites as values, and return a
bool
value
indicating if they
are
colliding.If
collided is not passed, all
sprites
must
have
a
...
collide.The
value
for each item in the dictionary is a list of the sprites
in the
second
group
it
collides
with.The two dokill arguments control if
the
sprites
from either group
...
that
collides
with with the given sprite.If there are no
collisions, then
this
returns
None.
...
you
don
't need all the features of the spritecollide function, this
function
will
be
a
